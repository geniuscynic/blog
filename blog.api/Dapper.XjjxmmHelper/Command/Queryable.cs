...
namespace Dapper.XjjxmmHelper.Command
{
    public class Queryable<T>
    {
        ...
        public Queryable(IDbConnection connection) : this(connection, new StringBuilder(), new WhereExpressionVisitor())
        {
            
        }

        public Queryable(IDbConnection connection, StringBuilder sql, WhereExpressionVisitor _whereVisitor)
        {
            this.sql = sql;
