<template>
  <div id="permission-menu-container">
    ...
    <el-table :data="buttons" style="width: 100%">
      ...
      <el-table-column label="按钮" width="360">
      ...
      <el-table-column label="所属菜单" width="180">
        <template slot-scope="scope">
          ...
          <div v-else>
            <el-form :inline="true">
              <el-form-item class="icon-input" label="">
                 <el-select v-model="scope.row.editMenuId" placeholder="请选择所属菜单">
    ...
    <el-dialog title="新增按钮" :visible.sync="dialogFormVisible">
      <el-form :model="form">
        ...
        <el-form-item label="按钮名" :label-width="formLabelWidth">
          <el-input v-model="form.name" auto-complete="off"></el-input>
        </el-form-item>
             <el-form-item label="按钮Code" :label-width="formLabelWidth">
...
<script>
import { API_REST_BUTTON } from "@/plugins/const";
...
export default {
  ...
  methods: {
      ...
      this.axios
        ...
        .then((response) => {
          ...
          for (let button of temps) {
            ...
            //debugger;
            button.menu2 = this.menus.find(
              (s) => s.id == button.menuId
            ).name;
    ...
    dialogYes() {
      ...
      this.axios
        .post(API_REST_BUTTON, this.form)
        .then((response) => {
          this.init();
          row.mode = 0;
          this.$message({
